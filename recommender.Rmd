...
```
```{r}
movies_random_selected <- slice(MovieLense_df, sample(1:n()))
movies_random_selected
```
```{r}
movies_random_selected <- slice(MovieLense_df, sample(1:n()))
movies_random_selected
...
train_test_split <- function(RRM, ratio=0.8){
  train_idx <- sample(x = c(TRUE, FALSE), size = nrow(RRM), replace = TRUE, 
      prob = c(ratio, 1 - ratio))
  
  RRM_train <- RRM[train_idx, ]
  RRM_test <- RRM[!train_idx, ]
  
  return (c(RRM_train, RRM_train))
}
...
```
```{r}
train_test_reduced[1]
```

2.Trainiere ein IBCF Modell mit 30 Nachbarn und Cosine Similarity
```{r}
Recommender_model_IBCF <- Recommender(data = train_test_reduced[1], method = "IBCF", parameter = list(k = 30, 
    method = "Cosine"))
...
similarity_lower <- replace(similarity_lower, 0.0, NA)
similarity_lower[similarity_lower == 0] <- NA  
ggplot(mapping = aes(x=similarity_lower)) +
  geom_histogram(fill="grey", na.rm=TRUE) + labs(title="Verteilung Aehnlichkeiten") + xlab("Cosine-Aehnlichkeit")
```
