...
```
Wir beobachten, dass nur wenige User über 400 Bewertungen abgeben haben. Bei den meisten Usern sind es nur ein paar duzend. Der Durchschnitt liegt etwa bei 100 Bewertungen.
...
```
Das Muster der Bewertungen unterschiedliche Genres ist sehr ähnlich. Die meisten Bewertungen liegen zwischen 3 und 4. Zudem gibt es starke Schwankungen bei der Anzahl der Bewertungen der jeweiligen Genres. Dies macht es schwieriger sie grafisch zu vergleichen.
...
summary(rating_std)
```
Eine Normalverteilung bei der Streuung des Ratings der individuellen Kunden.
...
Recommender_model_IBCF_cosine_top15 <-  predict(Recommender_model_IBCF_cosine, newdata=getData(eval_reduced, "unknown"), n=NUM_RECOMMENDATIONS)
Recommender_model_UBCF_cosine_top15 <-  predict(Recommender_model_UBCF_cosine, newdata=getData(eval_reduced, "unknown"), n=NUM_RECOMMENDATIONS)
Recommender_model_IBCF_jaccard_top15 <-  predict(Recommender_model_IBCF_jaccard, newdata=getData(eval_reduced, "unknown"), n=NUM_RECOMMENDATIONS)
Recommender_model_UBCF_jaccard_top15 <-  predict(Recommender_model_UBCF_jaccard, newdata=getData(eval_reduced, "unknown"), n=NUM_RECOMMENDATIONS)
```
...
Sowohl bei der Coverage, als auch bei der Novelty hat der Parameter N einen grossen einfluss. Falls man diese beiden maximieren möchte, sollte man ein mögichst hohes N wählen. Given beeinflusst die beiden Metriken fast gar nicht. Dies ist identisch bei beiden Modellen. Interessanterweise erreicht das SVD5 Modell aber einen höheren Wert. Im unteren Plot ist kann man die Summe der beiden Metriken ablesen.
