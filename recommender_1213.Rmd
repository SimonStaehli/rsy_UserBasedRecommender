...
given_ratings_in_cv <- -30 # all but
...
Der Hyperparameter _k_ dient zur Beschneidung der Matrizenzerlegung die dann zur Annäherung der Ratingmatrix genutzt wird. Desto grösser gewählt, desto besser der Fit auf den Trainingsdaten. 
...
vegan::vegdist(t(S), method = "jaccard" ,diag = TRUE, upper = TRUE)
````
```{r}
item_item_jaccard(S)
