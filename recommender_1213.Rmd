...
plot(ev, annotateesults, annotate=TRUE, average=TRUE)

```
**Beschreibung:**

Die beiden Plots zeigen ein ähnliches Resultat. Für das beste K hinsichtlich beider Metriken ist $k=5$.
...
# Check if snippet of arraay is equal
sum(as.integer(res_package[1:5,1:5] - (1-res_custom[1:5,1:5]))) == 0

````

**Beschreibung:**

Die beiden Funktionen ergeben fast das gleiche. Unsere Implementierung müsste noch mit der Funktion $Sim_{vegan}=1-Sim_{Custom}$ ergänzt werden. Wir nehmen an, dass dieses Package die Jaccard-Distanz abbildet.
