...
````{r}
# No sense min work example
foo <- sample_mat - sample_mat[, 1]

# Works hardcode number
colSums(foo == 0) / 700

# Works not from dim
colSums(foo == 0) / as.numeric(dim(foo[2]))

# Types are the same 
class(as.numeric(dim(foo[2])))
class(700)
...
````{r}
# Jaccard Sim
item_item_jaccard <- function(X, item_count){
  # Matches the Exact items as Equal
  #
  
  # Create empty matrix
  M <- Matrix(nrow = item_count, ncol = item_count)
  #X <- X > 0
  for (i in 1:item_count){
    tmp <- X - X[, i]
    M[i ,1:item_count] <- colSums(tmp == 0)/item_count
  }
  return(M)
}

foo <- item_item_jaccard(X = sample_mat, item_count = 700)

foo[1,1:5]

foo <- colSums(sample_mat == sample_mat[1])
...
jaccard_package <- proxy::dist(t(sample_mat), method="Jaccard", convert_similarities = TRUE)
jaccard_custom <- item_item_jaccard(X = sample_mat, item_count = 700)

jaccard_package[1:5]
jaccard_custom[1:5,1]
